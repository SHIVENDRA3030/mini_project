...
export const ChatListItem: React.FC<ChatListItemProps> = ({ chat, currentUser, onClick, isActive }) => {
  ...
  return (
    <div
      onClick={onClick}
      className={`flex items-center p-3 cursor-pointer transition-colors duration-200 border-b border-slate-800 ${
    ...
    >
      ...
      <div className="flex-1 overflow-hidden">
        <div className="flex justify-between items-center">
          <h3 className="font-semibold text-gray-100 truncate">{otherParticipant?.name}</h3>
          {lastMessage && (
            <span className={`text-xs ${unreadCount > 0 ? 'text-indigo-400 font-bold' : 'text-gray-400'}`}>
